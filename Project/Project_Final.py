...
from keras.layers import Convolution1D, AtrousConvolution1D, Flatten, Dense,     Input, Lambda, merge


# # Creating a function for making a discriminative model (D) which will filter the input between generstive model (G) and normal input(R)
...
# # Creating a function for making a generative model (G)
...
def get_training_data(frame_size, frame_shift):
    sr, audio = get_audio('bass_electronic_018-022-100.wav')
    X_train = []
    base = 0
...
if __name__ == '__main__':
    ...
    for i in range(n_epochs):
        ...
        for index in range(n_minibatches):
            ...
            X=np.column_stack((audio_batch,generated_audio))
            print(X.shape)
